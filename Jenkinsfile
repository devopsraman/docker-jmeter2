			  ...
			  steps {
				  sh '''
				        TEST_REPORT_FOLDER="${JOB_NAME}-${BUILD_NUMBER}"
                                       ...
                                       docker run --rm   -v /tmp/$TEST_REPORT_FOLDER/jmeter/:/tmp-jenkins   -v ${WORKSPACE}:/jmeter_tests jmeter -n -t /jmeter_tests/tests/MVP1.0MaxLTV.v2.jmx -l /tmp/$TEST_REPORT_FOLDER/jmeter/     
