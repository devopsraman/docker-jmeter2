			  ...
			  steps {
				  sh '''
				        TEST_REPORT_FOLDER="${JOB_NAME}-${BUILD_NUMBER}"
                                       ...
                                       /usr/local/bin/docker run -dit   -v /tmp/$TEST_REPORT_FOLDER/jmeter/:/tmp-jenkins --name master  lazzurs/jmeter  -v ${WORKSPACE}:/jmeter_tests jmeter -n -t /jmeter_tests/tests/MVP1.0MaxLTV.v2.jmx -l /tmp/$TEST_REPORT_FOLDER/jmeter/     
